...
export function AdminWorkflowPage() {
  ...
  useEffect(() => { void loadApplications().catch(() => setError('Unable to filter applications.')) }, [loadApplications])
  ...
  async function decide(id: string, status: typeof decisions[number]) {
    ...
    try {
      ...
      setMessage(`Application marked ${status}.`); await loadAll()
  ...
  async function saveTeam(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setMessage(selectedTeam ? 'Team updated.' : 'Team created.'); await loadAll()
  ...
  return <main className="mx-auto min-h-[65vh] max-w-7xl px-5 py-10 sm:px-8 sm:py-14">
    ...
    {error && <p className="mt-6 rounded-xl border border-red-300/30 bg-red-400/10 p-4 text-red-200" role="alert">{error}</p>}{message && <p className="mt-6 rounded-xl border border-emerald-300/30 bg-emerald-400/10 p-4 text-emerald-200" role="status">{message}</p>}
    ...
    <section className="mt-10"><h2 className="text-2xl font-black">Review applications</h2><div className="mt-4 grid gap-3 rounded-2xl border border-white/10 p-4 sm:grid-cols-2"><label className="font-semibold">Status<select className={`${inputClass} mt-2`} value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="">All statuses</option><option>Pending</option><option>Approved</option><option>Rejected</option><option>Waitlisted</option></select></label><label className="font-semibold">Project<select className={`${inputClass} mt-2`} value={projectFilter} onChange={(event) => setProjectFilter(event.target.value)}><option value="">All projects</option>{projects.map((project) => <option key={project.id} value={project.id}>{project.title}</option>)}</select></label></div>
      {applications.length === 0 ? <p className="mt-4 rounded-xl border border-white/10 p-5 text-slate-300">No applications match these filters.</p> : <div className="mt-4 grid gap-4 lg:grid-cols-2">{applications.map((item) => <article className="rounded-2xl border border-white/10 bg-white/[0.05] p-5" key={item.id}><p className="text-sm font-bold uppercase tracking-wider text-cyan-300">{item.status}</p><h3 className="mt-2 text-xl font-black">{item.projectTitle}</h3><p className="mt-2 break-all text-slate-300">{item.studentEmail}</p>{item.note && <p className="mt-3 rounded-xl bg-slate-950 p-3 text-slate-300">{item.note}</p>}<div className="mt-4 flex flex-wrap gap-2">{decisions.map((status) => <button className="min-h-11 rounded-lg border border-white/20 px-4 font-bold hover:bg-white/10" key={status} onClick={() => void decide(item.id, status)}>{status}</button>)}</div></article>)}</div>}
    </section>
    ...
    <section className="mt-10 rounded-2xl border border-white/10 bg-white/[0.05] p-5 sm:p-8"><h2 className="text-2xl font-black">Create or manage a team</h2><form className="mt-5 grid gap-5" onSubmit={saveTeam}>
      ...
      <fieldset><legend className="font-semibold">Approved members</legend>{approvedCandidates.length === 0 ? <p className="mt-3 text-slate-400">Approve applications for this project before creating its team.</p> : <div className="mt-3 grid gap-2 md:grid-cols-2">{approvedCandidates.map((item) => <label className="flex min-h-12 items-center gap-3 rounded-xl bg-slate-950 px-4" key={item.studentId}><input checked={memberIds.includes(item.studentId)} type="checkbox" onChange={() => toggleMember(item.studentId)} />{item.studentEmail}</label>)}</div>}</fieldset>
      <label className="font-semibold">Team leader<select className={`${inputClass} mt-2`} required value={leaderId} onChange={(event) => setLeaderId(event.target.value)}><option value="">Choose a selected member</option>{approvedCandidates.filter((item) => memberIds.includes(item.studentId)).map((item) => <option key={item.studentId} value={item.studentId}>{item.studentEmail}</option>)}</select></label>
